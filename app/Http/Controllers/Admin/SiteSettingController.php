...
class SiteSettingController extends Controller
{
    ...
    {
        $keys = [];
        foreach ($this->contentGroups as $group) {
            foreach ($group["groups"] as $fields) {
    ...
    public function index(): View
    {
        ...
        $activeTab = request("tab", "homepage");
    ...
    public function edit(): View
    {
        ...
        $groups = [];
        foreach ($this->contentGroups as $group) {
            foreach ($group["groups"] as $name => $fields) {
    ...
    {
        $tab = $request->input("tab", "homepage");
        if (!isset($this->contentGroups[$tab])) {
            return redirect()->route("admin.content.index")->with("error", "Invalid content tab.");
        }

        $group = $this->contentGroups[$tab];
        $keys = [];
        foreach ($group["groups"] as $fields) {
            foreach ($fields as $field) $keys[] = $field["key"];
        }

        $data = [];
        foreach ($keys as $key) $data[$key] = $request->input($key);
        SiteSetting::setMany($data);

        return redirect()->route("admin.content.index", ["tab" => $tab])
            ->with("status", ucfirst($group["name"]) . " settings saved successfully.");
    }
}
